...
RSpec.describe RecipeFoodsController, type: :request do
  ...
  describe 'POST #create' do
    it 'creates a new recipe_food' do
      recipe = Recipe.create(name: 'Sample Recipe', description: 'Sample Description', preparation_time: 30, cooking_time: 60, user: @user)
      food = Food.create(name: 'Sample Food', measurement_unit: 'grams', price: 2.99, quantity: 100, user: @user)
      post user_recipe_recipe_foods_path(user_id: @user.id, recipe_id: recipe.id), params: { recipe_food: { quantity: 1, food_id: food.id, recipe_id: recipe.id } }
